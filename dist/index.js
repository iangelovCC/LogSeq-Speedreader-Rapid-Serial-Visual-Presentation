...
(() => {
      ...
      !function(e, t) {
        ...
        return (() => {
          ...
          const sr = function(e3, t3) {
            for (var n2 = 0, r2 = (t3 = or(t3, e3)).length; null != e3 && n2 < r2; )
              e3 = e3[ir(t3[n2++])];
  ...
  // src/rsvp.ts
  var MARKDOWN_LINK = /\[([^\]]+)]\(([^)]+)\)/g;
  var MARKDOWN_IMAGE = /!\[([^\]]*)]\(([^)]+)\)/g;
  var CODE_FENCE = /```[\s\S]*?```/g;
  var INLINE_CODE = /`[^`]*`/g;
  var HTML_TAG = /<\/?[^>]+>/g;
  var EMPHASIS_BOLD = /(\*\*|__)(.*?)\1/g;
  var EMPHASIS = /(\*|_)(.*?)\1/g;
  var HEADING_OR_LIST = /^\s{0,3}(#{1,6}|\*|\-|\+|\d+\.)\s+/gm;
  var BLOCKQUOTE = /^\s{0,3}>\s?/gm;
  function normalizeText(text) {
    return text.replace(CODE_FENCE, " ").replace(MARKDOWN_IMAGE, "$1").replace(MARKDOWN_LINK, "$1").replace(INLINE_CODE, " ").replace(EMPHASIS_BOLD, "$2").replace(EMPHASIS, "$2").replace(HTML_TAG, " ").replace(HEADING_OR_LIST, "").replace(BLOCKQUOTE, "").replace(/\s+/g, " ").trim();
  }
  ...
  function getSentenceStartIndices(words) {
    const indices = [0];
  ...
  function findSentenceBoundary(words, currentIndex, direction) {
  ...
  function computeDelayMs(word, baseMs, settings, index, total) {
    let delay = baseMs;
...
`;
  function renderTemplate(state2) {
    const progress = state2.total > 0 ? Math.min(100, Math.max(0, state2.index / state2.total * 100)) : 0;
    const word = escapeHtml(state2.word || "");
    const scheme = state2.colorScheme === "auto" ? "" : state2.colorScheme;
    return `
  <div id="rsvp-root" class="${state2.visible ? "show" : ""}">
    <div class="rsvp-panel ${scheme}">
      <div class="rsvp-header">
        <div><strong>RSVP Speed Reader</strong></div>
        <div>${escapeHtml(state2.status)}</div>
      </div>
      <div class="rsvp-word">${word}</div>
      <div class="rsvp-controls">
        <button class="rsvp-button primary" data-on-click="togglePlay">${state2.paused ? "Start" : "Pause"}</button>
      ...
      <div class="rsvp-meta">${state2.index + 1} / ${state2.total} \xB7 ${state2.wpm} WPM</div>
    </div>
  </div>
  `;
  }
  ...
  var getSettings = () => {
    ...
    return {
      ...DEFAULT_SETTINGS,
      ...settings,
      wpm: clamp(Number(settings.wpm ?? DEFAULT_SETTINGS.wpm), 50, 1e4)
    };
  };
  var updateUi = () => {
...
:root { --rsvp-font-size: ${settings.fontSize}px; --rsvp-font-family: ${settings.fontFamily}; }`
  ...
  var adjustWpm = (delta) => {
    const settings = getSettings();
    const nextWpm = clamp(settings.wpm + delta, 50, 1e4);
    logseq.updateSettings({ wpm: nextWpm });
    updateUi();
  };
  var normalizeShortcut = (value) => value.trim().toLowerCase();
  ...
  var getSelectedText = async () => {
    ...
    if (selectedBlocks && selectedBlocks.length > 0) {
      return blocksToText(selectedBlocks);
    }
    return null;
  };
  var getCurrentPageText = async () => {
