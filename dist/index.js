...
(() => {
  ...
  var RSVPPlayer = class {
    constructor() {
      this.words = [];
      this.index = 0;
      this.running = false;
      this.timer = null;
      this.wpm = DEFAULT_SETTINGS.wpm;
      ...
      this.onKeyDown = (event) => {
        ...
        const matches = (configured) => key === configured || event.key === configured;
    ...
    maxWpm() {
      return clamp(readNumberSetting("maxWpm", DEFAULT_SETTINGS.maxWpm), 100, 1e4);
    }
    tick() {
      this.clearTimer();
    ...
    buildUI() {
      ...
      this.controlsEl.textContent = "Toggle \u2022 Stop \u2022 Sentence \xB1 \u2022 WPM \xB1 (keys configurable in plugin settings)";
      this.overlay.append(this.wordEl, this.infoEl, this.progressEl, this.controlsEl);
      document.body.appendChild(this.overlay);
      this.applyTheme();
    }
    applyTheme() {
      const requested = logseq.settings?.theme ?? DEFAULT_SETTINGS.theme;
      ...
      if (dark) {
        ...
        const progressInner = this.progressEl.firstElementChild;
  ...
  function normalizeText(input) {
    return input.replace(/```[\s\S]*?```/g, " ").replace(/`[^`]*`/g, " ").replace(/!\[[^\]]*\]\([^)]*\)/g, " ").replace(/\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/\[\[([^\]]+)\]\]/g, "$1").replace(/\(\(([^)]+)\)\)/g, "$1").replace(/(^|\s)#([\w/-]+)/g, " $2").replace(/[*_~>#`|-]+/g, " ").replace(/\s+/g, " ").trim();
  }
  function splitWords(text) {
    if (!text) return [];
    return text.split(/\s+/).map((word) => word.trim()).filter(Boolean);
  }
  function getPunctuationFactor(word, mode) {
  ...
  var player = new RSVPPlayer();
  function settingsSnapshot() {
    const settings = { ...DEFAULT_SETTINGS, ...logseq.settings ?? {} };
    settings.maxWpm = clamp(Number(settings.maxWpm || DEFAULT_SETTINGS.maxWpm), 100, 1e4);
  ...
  async function readSingleBlock(uuid) {
    if (!uuid) return;
  ...
  async function main() {
    ...
    logseq.App.registerUIItem("toolbar", {
      ...
      template: '<a class="button" data-on-click="rsvpReadSelectedBlocks" title="RSVP speed-reader">RSVP</a>'
    });
